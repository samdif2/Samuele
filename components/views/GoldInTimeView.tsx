...
const GoldInTimeView: React.FC<GoldInTimeViewProps> = ({ capital, setCapital, years, setYears }) => {
  ...
  const yearlyData: YearlyData[] = useMemo(() => {
    ...
    let runningAccumulatedRevenue = 0;

    for (let i = 1; i <= years; i++) {
      // Logic for Gold in Time:
      // Capital increases every year (Recurring investment) -> Year 1: 1*Cap, Year 2: 2*Cap
      const currentCapital = safeCapital * i;
      
      // Revenue is % of the current accumulated capital
      const yearlyRevenue = Math.round(currentCapital * (percentage / 100));
      
      // Accumulo is the sum of all revenues generated so far
      runningAccumulatedRevenue += yearlyRevenue;
      ...
      data.push({
        ...
        accumulated: runningAccumulatedRevenue,
      });
    }
    return data;
  }, [capital, years, percentage]);

  // Total Invested Capital (Input * Years)
  const totalInvestedCapital = (capital || 0) * years;
  
  // Total Revenue Generated
  const totalRevenue = yearlyData.length > 0 ? yearlyData[yearlyData.length - 1].accumulated : 0;
  
  // Total Value (Invested + Revenue)
  const totalValue = totalInvestedCapital + totalRevenue;
  ...
  const handleExportPDF = useCallback(() => {
    setIsExporting(true);
    const reportElement = document.getElementById('pdf-export-area-time');
    if (!reportElement) {
    ...
    leftHeaderText.innerHTML = `Piano personalizzato per un Gold in Time da <strong>${formatCurrency(capital)}</strong> per <strong>${years}</strong> anni.`;
    ...
    const filename = `Gold_in_Time_${capital}x${years}.pdf`;
  ...
  return (
    <div className="p-4 space-y-6">
      <h2 className="text-2xl font-bold text-[#D4AF37] text-center">Gold in Time</h2>
      <div className="bg-white p-4 rounded-lg shadow-sm">
        <div className="space-y-4">
          {/* Capital Input with Dynamic Formatting */}
          <div className="flex items-center space-x-4">
            <label htmlFor="capital-time" className="text-sm font-medium text-gray-700 whitespace-nowrap">Capitale</label>
            <input
              type="text"
              inputMode="numeric"
              id="capital-time"
          ...
          <div className="flex items-end space-x-4">
            <div className="w-1/2">
                <label htmlFor="years-time" className="block text-sm font-medium text-gray-700">Durata (Anni)</label>
      ...
      <div id="pdf-export-area-time">
        <div className="bg-[#D4AF37] text-white p-6 rounded-lg shadow-lg text-center">
            ...
            <p className="text-sm opacity-90">{formatCurrency(totalInvestedCapital)} (Capitale) + {formatCurrency(totalRevenue)} (Ricavi)</p>
        </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 px-4">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full text-center">
            <p className="text-gray-800 font-medium mb-6">
              Il Gold in Time prevede un deposito minimo di 1.200 euro all'anno.
            </p>
            <button
